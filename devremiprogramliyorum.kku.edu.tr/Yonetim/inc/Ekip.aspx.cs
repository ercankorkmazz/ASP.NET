...
public partial class Yonetim_inc_Ekip : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Page.IsPostBack == false)
        {
            ...
            bilgiGetir.verileriGetir("select projeEkibi from sayfalar where id='1'");
            if (bilgiGetir.SQLTablo.Rows.Count > 0)
            {
                projeEkibiTXT.InnerText = bilgiGetir.SQLTablo.Rows[0][0].ToString();
            }
        }
    }
    protected void kaydetBTN_Click(object sender, EventArgs e)
    {
        ...
        if (sayfaGuncelle.komutCalistir("update sayfalar set projeEkibi='" + projeEkibiTXT.InnerText + "' where id='1'") == "1")
        {
