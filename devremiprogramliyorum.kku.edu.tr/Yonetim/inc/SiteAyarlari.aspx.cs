...
public partial class Yonetim_inc_SiteAyarlari : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Page.IsPostBack == false)
        {
            ...
            bilgiGetir.verileriGetir("select title,bannerText from sayfalar where id='1'");
            if (bilgiGetir.SQLTablo.Rows.Count > 0)
            {
                titleTXT.Text = bilgiGetir.SQLTablo.Rows[0][0].ToString();
                bannerTextTXT.Text = bilgiGetir.SQLTablo.Rows[0][1].ToString();
            }
        }
    }
    protected void kaydetBTN_Click(object sender, EventArgs e)
    {
        ...
        if (sayfaGuncelle.komutCalistir("update sayfalar set title='" + titleTXT.Text + "', bannerText='" + bannerTextTXT.Text + "' where id='1'") == "1")
        {
