...
public partial class Yonetim_inc_duyurular : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Page.IsPostBack == false)
        {
            ...
            bilgiGetir.verileriGetir("select duyurular from sayfalar where id='1'");
            if (bilgiGetir.SQLTablo.Rows.Count > 0)
            {
                duyurularTXT.InnerText = bilgiGetir.SQLTablo.Rows[0][0].ToString();
            }
        }
    }
    protected void kaydetBTN_Click(object sender, EventArgs e)
    {
        ...
        if (sayfaGuncelle.komutCalistir("update sayfalar set duyurular='" + duyurularTXT.InnerText + "' where id='1'") == "1")
        {
