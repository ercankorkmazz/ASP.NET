...
public partial class Yonetim_inc_katilimFormu : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Page.IsPostBack == false)
        {
            ...
            bilgiGetir.verileriGetir("select katilimFormu from sayfalar where id='1'");
            if (bilgiGetir.SQLTablo.Rows.Count > 0)
            {
                katilimFormuTXT.InnerText = bilgiGetir.SQLTablo.Rows[0][0].ToString();
            }
        }
    }
    protected void kaydetBTN_Click(object sender, EventArgs e)
    {
        ...
        if (sayfaGuncelle.komutCalistir("update sayfalar set katilimFormu='" + katilimFormuTXT.InnerText + "' where id='1'") == "1")
        {
