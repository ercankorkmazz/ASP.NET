...
public partial class inc_ogrenci_video : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        int videoID = 2018;
        if (Session[Request.Url.Authority + "AGrubuVideoID"] != null)
            videoID = Convert.ToInt32(Session[Request.Url.Authority + "AGrubuVideoID"]);
    ...
    protected void rptvideolar_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        ...
        rp.DataSource = yi.yorumCekOgrenciID(2007);
        rp.DataBind();
        rp.Dispose();
        ...
        vb.DataSource = videoButonlari.digerVideoListeleOgrID(2007);
        vb.DataBind();
        vb.Dispose();
    }
    protected void rptVideoGitButonlari_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        ...
        {
            Session[Request.Url.Authority + "AGrubuVideoID"] = Convert.ToInt32(e.CommandArgument.ToString());
            akranvideogetir vi = new akranvideogetir();
            rptDigerVideolar.DataSource = vi.akranVideoListeleVideoID(Convert.ToInt32(Session[Request.Url.Authority + "AGrubuVideoID"]));
            rptDigerVideolar.DataBind();
        }
    }
}